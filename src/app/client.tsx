...
function HomeClient({
  books,
  readList: initialReadList,
}: {
  books: Book[];
  readList: Set<Book["ISBN"]>;
}) {
  const [readList, setReadList] = useState<Set<Book["ISBN"]>>(initialReadList);
...
export default function HomeClientContainer({books}: {books: Book[]}) {
  const readList = use(api.readList.fetch());

  return <HomeClient books={books} readList={readList} />;
}
